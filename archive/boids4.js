    ...
    cohesion(boidArr) {
        ...
        for (let other of boidArr){ //this is where you check ALL of list. good place to implement quadtree
            ...
            if (other != self && d < perceptionRadius ){
                ...
                /////////////////// COLOR ///////////////////
                if (total > 1){
                    avgColR.push(other.colorR);
                    avgColG.push(other.colorG);
                    avgColB.push(other.colorB);
                    console.log('working');
                }
                else{
        ...
        if(total > 0){
            ...
            for (let colNum of avgColR){
                sumR += colNum;
            }

            for (let colNum of avgColG){
                sumG += colNum;
            }

            for (let colNum of avgColB){
                sumB += colNum;
            }

            this.colorR = int(sumR / total);
            this.colorG = int(sumG / total);
            this.colorB = int(sumB / total);
        }
        return avg;
    }
